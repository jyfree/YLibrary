...
object FastBlur {
    private val TAG = "FastBlur"

    fun blur(context: Context, sentBitmap: Bitmap, radius: Int, canReuseInBitmap: Boolean): Bitmap {
        return if (SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            rsBlur(context, sentBitmap, radius)
        } else {
            fastBlur(sentBitmap, radius, canReuseInBitmap)
        }
    }
