...
import com.jy.sociallibrary.ext.pay.SDKPayCommonActivity
import com.jy.sociallibrary.wx.WXPayBean

class PaySimpleCommonActivity : SDKPayCommonActivity() {

    companion object {
        fun startAct(context: Context) {
            ActivityUtils.startActivity(context, PaySimpleCommonActivity::class.java)
        }
    }
    ...
    fun onClickPay(view: View) {
        when (view.id) {
            ...
            R.id.pay_2wx -> {
                ...
                wxPayOrderSuccess(wxPayVo)
            }
        }
    }

    override fun onPaySuccess(type: Int, orderId: String?) {
        YLogUtils.i("支付成功--类型：", type, "orderId", orderId)
    }

    override fun onPayFail(type: Int, error: String?) {
        YLogUtils.e("支付失败--类型：", type, "error", error)
    }

    override fun onPayCancel(type: Int) {
