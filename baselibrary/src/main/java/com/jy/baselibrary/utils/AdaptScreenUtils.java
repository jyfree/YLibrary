...
/**
 * 使用说明：
 * 1.在xml中按照设计图（必须按照设计图分辨率设置）设置尺寸
 * 2.调用adaptWidth | adaptHeight 进行宽度|高度适配，参数为设计图尺寸宽|高
 * 3.重写activity#getResources()方法，使用调用adaptWidth | adaptHeight 后的Resource替代系统Resource
 * 3.在Activity#OnDestory中必须调用closeAdapt关闭适配，防止其它页面出现问题
 */
public class AdaptScreenUtils {
    ...
    public static int pt2Px(final float ptValue) {
        DisplayMetrics metrics = BaseUtils.getApp().getResources().getDisplayMetrics();
    ...
    private static void applyDisplayMetrics(final Resources resources, final float newXdpi) {
        ...
        BaseUtils.getApp().getResources().getDisplayMetrics().xdpi = newXdpi;
    ...
    private static void applyMetricsFields(final Resources resources, final float newXdpi) {
        for (Field metricsField : sMetricsFields) {
            try {
                ...
                YLogUtils.INSTANCE.e("AdaptScreenUtils", "applyMetricsFields: " + e);
            }
        }
    }
    ...
    private static DisplayMetrics getMetricsFromField(final Resources resources, final Field field) {
        try {
            ...
            YLogUtils.INSTANCE.e("AdaptScreenUtils", "getMetricsFromField: " + e);
            return null;
        }
    }
}
